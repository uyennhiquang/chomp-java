...
public class Chomp {
  ...
  /**
   * Creates a game of chomp. Any user interface who wishes to play this game must
   * implement their own play() method.
   * Chomp is a grid-based 2-player combinatorial game where the objective is to
   * make the other player eat the poisoned piece (in this iteration it will
   * always be the top left tile). Players take turn "chomp" a tile on the board
   * -- when a tile is chomped, all the tiles to the right and bottom of that
   * piece will also be chomped, forming a filled mini-rectangle.
   * The board size is arbitrary. It is up to the programmer's discretion to
   * handle any errors that might rise from the user of this method inputting
   * illegal values of rows and cols.
   * 
   * @param rows
   * @param cols
   */
  ...
  /**
   * When a tile is chomped, all the tiles to the right and bottom of that piece
   * will also be chomped, forming a filled mini-rectangle.
   * 
   * @param row
   * @param col
   * @return
  ...
  public ChompStatus chomp(int row, int col) throws ChompException {
    ...
    // the entire column right of col is chomped in row, then moves on to the next
    // row all the way until this.rows
